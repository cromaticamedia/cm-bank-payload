...
//     e.preventDefault()
//     toast.loading(t.redirectingToDashboard, { id: 'dashboard-redirect' })
//     setTimeout(() => {
//       router.push(`/admin?returnTo=${pathname}`)
//     }, 600)
//   }
...
export default function AppSidebar({ locale, theme, onApplyTheme }: AppSidebarProps) {
  const pathname = usePathname()
  const router = useRouter()
  const t = useTranslations(translations, locale)
  const [collapsed, setCollapsed] = useState(true)
